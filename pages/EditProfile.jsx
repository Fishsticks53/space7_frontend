...
export default function EditProfile() {
  ...
  const onSaveUsername = async () => {
    ...
    try {
      ...
      Alert.alert("Save failed", error?.message || "Unable to update username.");
    } finally {
  ...
  const onChangePassword = async () => {
    ...
    try {
      ...
      Alert.alert("Save failed", error?.message || "Unable to update password.");
    } finally {
  ...
  const onPickImage = async () => {
    ...
    if (!permission.granted) {
      Alert.alert("Permission required", "Allow gallery access to update profile picture.");
      return;
    }
  ...
  const onSavePicture = async () => {
    ...
    try {
      ...
      Alert.alert("Save failed", error?.message || "Unable to update profile picture.");
    } finally {
    ...
    <Screen>
      <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
        <View style={styles.card}>
          ...
          <Text style={styles.subtitle}>Manage your account settings in one place</Text>

          <TouchableOpacity style={styles.sectionHeader} onPress={() => toggle("username")}>
            <Text style={styles.sectionTitle}>Change Username</Text>
            <Text style={styles.sectionArrow}>{isExpanded("username") ? "-" : "+"}</Text>
          </TouchableOpacity>
          {isExpanded("username") ? (
            <View style={styles.sectionBody}>
              ...
              />
              <TouchableOpacity
                style={[styles.saveButton, (!changedUsername || savingUsername) && styles.saveButtonDisabled]}
                onPress={onSaveUsername}
                disabled={!changedUsername || savingUsername}
              >
                <Text style={styles.saveButtonText}>{savingUsername ? "Saving..." : "Update Username"}</Text>
              </TouchableOpacity>
            </View>
          ) : null}

          <TouchableOpacity style={styles.sectionHeader} onPress={() => toggle("bio")}>
            <Text style={styles.sectionTitle}>Change Bio</Text>
            <Text style={styles.sectionArrow}>{isExpanded("bio") ? "-" : "+"}</Text>
          </TouchableOpacity>
          {isExpanded("bio") ? (
            <View style={styles.sectionBody}>
              <TextInput
              ...
              />
              <TouchableOpacity
                style={[styles.saveButton, (!changedBio || savingBio) && styles.saveButtonDisabled]}
                onPress={onSaveBio}
                disabled={!changedBio || savingBio}
              >
                <Text style={styles.saveButtonText}>{savingBio ? "Saving..." : "Update Bio"}</Text>
              </TouchableOpacity>
            </View>
          ) : null}

          <TouchableOpacity style={styles.sectionHeader} onPress={() => toggle("password")}>
            <Text style={styles.sectionTitle}>Change Password</Text>
            <Text style={styles.sectionArrow}>{isExpanded("password") ? "-" : "+"}</Text>
          </TouchableOpacity>
          {isExpanded("password") ? (
            <View style={styles.sectionBody}>
              <TextInput
              ...
              >
                <Text style={styles.saveButtonText}>{savingPassword ? "Saving..." : "Update Password"}</Text>
              </TouchableOpacity>
            </View>
          ) : null}

          <TouchableOpacity style={styles.sectionHeader} onPress={() => toggle("picture")}>
            <Text style={styles.sectionTitle}>Change Profile Picture</Text>
            <Text style={styles.sectionArrow}>{isExpanded("picture") ? "-" : "+"}</Text>
          </TouchableOpacity>
          {isExpanded("picture") ? (
            <View style={styles.sectionBody}>
              ...
              <TouchableOpacity
                style={[styles.saveButton, (!pickedImage || savingPicture) && styles.saveButtonDisabled]}
                onPress={onSavePicture}
                disabled={!pickedImage || savingPicture}
              >
                <Text style={styles.saveButtonText}>{savingPicture ? "Saving..." : "Update Picture"}</Text>
              </TouchableOpacity>
            </View>
          ) : null}

          <TouchableOpacity style={styles.sectionHeader} onPress={() => toggle("delete")}>
            <Text style={[styles.sectionTitle, styles.dangerTitle]}>Delete Account</Text>
            <Text style={styles.sectionArrow}>{isExpanded("delete") ? "-" : "+"}</Text>
          </TouchableOpacity>
          {isExpanded("delete") ? (
            <View style={styles.sectionBody}>
              ...
              <TouchableOpacity
                style={[styles.deleteButton, deletingAccount && styles.saveButtonDisabled]}
